				...
				for i := 0; i < n; i++ {
					...
					if err != nil {
						...
						for _, prefix := range temp_prefixlist[prod] {
							...
							// update route
							interest := ndn.MakeInterest(ndn.ParseName("update"), []byte(fmt.Sprintf("%s,%d,%d,%d", prefix, best.Distance, network[cons][best.Path[1]].Fce, 300000)), ndn.ForwardingHint{ndn.ParseName(temp_facelist[router].Tkn), ndn.ParseName("update")})
							interest.MustBeFresh = true
